...
template<class T>
void
Context::gather_impl(Tag<T>, const T& data, std::vector<T>& recvData) const {
  if (isRootProc()) {
...
template<class T>
void
Context::gather_impl(Tag<std::vector<T> >, const std::vector<T>& data, std::vector<std::vector<T> >& recvData) const {
  ...
  if (isRootProc()) {
    ...
    auto nbElem = std::accumulate(recvData.begin(), recvData.end(), 0, [](size_t sum, const std::vector<T>& data) { return sum + data.size(); });
    total.resize(nbElem);
  }
...
template<class T>
void
Context::broadcast_impl(Tag<T>, T& data) const {
